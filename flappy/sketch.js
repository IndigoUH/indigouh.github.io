...
function preload() {
    bgImage = loadImage('graphics/underwater.png')
    groundImage = loadImage('graphics/ground.png')
    birdImage = loadImage('graphics/mrkrabsundies.png')
    pipeImage = loadImage('graphics/Planktoooon copy.png')
    flappyFont = loadFont('fonts/flappy.ttf')
    gameFont = loadFont('fonts/font.ttf')

    jump = loadSound('sounds/HUH.mp3')
    score = loadSound('sounds/score.wav')
    explosion = loadSound('sounds/failure.mp3')
    hurt = loadSound('sounds/hurt.wav')
    music = loadSound('sounds/grassmusic.mp3')
}
...
function title() {
    ...
    textAlign(CENTER)
    textFont(flappyFont)
    text("Poopy Bird", width / (2 * 1.74), 100)
    textSize(14)
